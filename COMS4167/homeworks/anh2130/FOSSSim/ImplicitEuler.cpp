...
*/
        int x_size = x.size();
		assert(x_size%2 == 0);
		int count = scene.getNumParticles();
		assert(2*count == x_size);
        scalar norm = 0.0;
        VectorXs dx = dt*v;
        ...
        do{
            ...
            if(scene.isFixed(i))
            {
                ...
                left.block(2*i,0,2,count).setZero();
                left.block(0,2*i,count,2).setZero();
			...
			// Check the residual from the linear solve
            #ifdef DEBUG
            VectorXs linslvrsd = left*dv_step-right;
            scalar lininfnorm = linslvrsd.array().abs().maxCoeff();
            if( lininfnorm > 1.0e-9 )
            {
                std::cout << outputmod::startred << "WARNING IN IMPLICITEULER: " << outputmod::endred;
            ...
            // Check the behavior of fixed DoFs
            #ifdef DEBUG
            for( int i = 0; i < count; ++i ) if( scene.isFixed(i) )
            {
                assert( dv_step(2*i) == 0.0 );
                assert( dv_step(2*i+1) == 0.0 );
            }
            #endif
